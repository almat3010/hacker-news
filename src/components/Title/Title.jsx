		...
		<>
			...
			<div key={article.id} className="title">
				...
				<a
					...
					href={article.url}
				>
					<div className="title__header">{article.title}</div>
				</a>
				<div className="title__author">by {article.by}</div>
				<div className="title__date">
					<img src={time} alt="icon-time" />
					{transformDate}
				</div>
				<div className="title__comments">Comments: {article.descendants}</div>
			</div>
		</>
	);
};
